...
public final class UnzipResultsHandlerTest {
  ...
  /**
   * Verifies that a GET request for a file within a results.zip file that
   * exists is successful, where the file is one of the BW plugins's .log
   * files.  Verifies that the response {@code Content-Type} is {@code
   * text/plain; charset=utf-8}, since that is the correct type for the BW
   * plugins's .log files, and since standard {@code Content-Type} detection
